<%@ 
tag import="org.zebrafish.chart.RadarChart" %><%@ 
...
attribute name="spline" type="java.lang.Boolean" description="spline" %><% 
		try {
			removeAllAttributes(request);
			RadarChart chart = new RadarChart(width, height);
			if(spline != null) {
				chart.setSpline(spline);
			}
